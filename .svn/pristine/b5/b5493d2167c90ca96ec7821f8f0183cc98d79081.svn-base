 ...
 *
 */
public class Page3D extends PageTurnning {
	...
	public void setBitmaps(Bitmap currentImage, Bitmap foreImage, Bitmap bgImage) {
		...
		super.setBitmaps(currentImage, foreImage, bgImage);
	}
	...
	public Page3D(Context context, AttributeSet attrs) {
		super(context, attrs);		
	}
	...
	private void calcPoints() {
		...
		// 如果继续翻页，会出现BUG故在此限制
		if (mTouch.x > 0 && mTouch.x < screenWidth) {
			if (mBezierStart1.x < 0 || mBezierStart1.x > screenWidth) {
				if (mBezierStart1.x < 0)
					mBezierStart1.x = screenWidth - mBezierStart1.x;

				float f1 = Math.abs(mCornerX - mTouch.x);
				float f2 = screenWidth * f1 / mBezierStart1.x;
	...
	public void drawCurrentPageShadow(Canvas canvas) {
		...
		if (mBezierControl2.y < 0)
			temp = mBezierControl2.y - screenHeight;
		else
	...
	private void startAnimation(int delayMillis) {
		...
		if (mCornerX > 0) {
			...
			dx = (int) (screenWidth - mTouch.x + screenWidth);
		}
		if (mCornerY > 0) {
			dy = (int) (screenHeight - mTouch.y);
		} else {
	...
	public boolean canDragOver() {
		if (mTouchToCornerDis > screenWidth / 10)
			return true;
		return false;
	}
